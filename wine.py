...
with mlflow.start_run(run_name="hyperparameter-sweep"):
    ...
    # ... (Code from your script above, ending with the mlflow.log_metrics block) ...
 
    print("\nOptimization complete. Finding and registering the best model...")

    # Get the ID of the parent run we are currently in
    parent_run_id = mlflow.active_run().info.run_id
    
    # Search for the best nested run *within this parent run*
    best_run = mlflow.search_runs(
        experiment_names=[experiment_name],
        filter_string=f"tags.'mlflow.parentRunId' = '{parent_run_id}'", # Filter for nested runs
        order_by=["metrics.val_rmse ASC"], # Sort by our metric
        max_results=1 # Get only the best one
    ).iloc[0] # Get the first row (which is the best run)

    best_run_id = best_run["run_id"]
    best_rmse_from_run = best_run["metrics.val_rmse"]
    
    print(f"\nBest trial found:")
    print(f"  Run ID: {best_run_id}")
    print(f"  Validation RMSE: {best_rmse_from_run}")

    # Log this to the parent run for easy access
    mlflow.set_tag("best_nested_run_id", best_run_id)

    # Register the model from the best run
    model_uri = f"runs:/{best_run_id}/model"
    model_name = "WineQualityModel_Hyperopt"
    
    mlflow.register_model(model_uri, model_name)
    
    print(f"\nSuccessfully registered model '{model_name}' (Version 1) from run {best_run_id}")

# The 'with mlflow.start_run(...)' block ends here
